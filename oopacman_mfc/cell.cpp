...
void Cell::draw_at(CDC &dc, int x, int y, ObjType ot)
{
    static bool pacman_eating = false;
    if (ot == PACMAN)
    {
        ImageList_Draw(
...
void Cell::draw(CDC &dc, int flags)
{
...
void Cell::redraw()
{
    ...
    (COOPacManView::GetView())->RedrawWindow(&rect, NULL, RDW_INVALIDATE | RDW_UPDATENOW);
}

bool Cell::processMessage(int message, Data extra_data)
{
    if( message == MSG_DRAW )
        draw(*(CDC*)extra_data.pointer(), 0);
    return false;
}
